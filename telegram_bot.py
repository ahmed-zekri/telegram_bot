...
def copy_data_files_to_executable_dir(filename):
    if os.path.exists(filename):
        ...
        if len(session_id) == 1:
            ...
            c = TelegramClient(f'session_name.{session_id[0]}', int(api_id[int(session_id[0])]),
        ...
        for username in usernames:
            ...
            while True:
                try:
                    ...
                    client = TelegramClient(f'session_name.{account_index}', int(api_id[account_index]),
...
if __name__ == '__main__':
    ...
    client = TelegramClient(f'session_name.{account_index}', int(api_id[account_index]), api_hash[account_index])
    client.start()
